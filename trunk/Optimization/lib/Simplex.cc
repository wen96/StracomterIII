...
Simplex::operator=(const Simplex &simplex) {
	...
	if (this != &simplex) {
		...
		/*for(int i = 0; i < simplex.Base.size(); i++)
			////cout << "BASE: " << simplex.Base[i] << endl;*/

		Base = simplex.Base;

		/*for(int i = 0; i < simplex.Base.size(); i++)
			////cout << "Nueva BASE: " << Base[i] << endl;*/
...
int Simplex::SimplexInt(void) {
	...
	int iter=0;
	while (NV.size() > 0 && (!optimo) && iter<30)
	{
		////cout<<"me he quedado pillado: "<< NV.size()<<endl;
		...
		////cout<<"tamaño varFloat: "<<vFloat.size()<<endl<<"Primera: "<<vFloat[0]<<endl;
...
Rational Simplex::getXi(int i)
{
	Rational tmp = Rational(0);
	for (int j = 1; j <= soluciones->getF(); j++)
	{
		if(Base[j-1] == i)
		{
			tmp=(*soluciones)(j, 1);
