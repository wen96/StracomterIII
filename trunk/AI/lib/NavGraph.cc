...
vector<ASNode> NavGraph::getAdyacence(int id) {
	...
	if (found){
		...
		//Creamos el vector de punteros con la adyacencia del nodo
		for (unsigned int i = 0; i < ady.size(); i++){
			//if(this->getNode(ady[i]).isEnabled()){
...
void NavGraph::writeFile(string name){
	TiXmlDocument doc;
	TiXmlDeclaration * decl = new TiXmlDeclaration( "1.0", "", "" );
	doc.LinkEndChild( decl );

	TiXmlElement * navg = new TiXmlElement( "navgraph" );
	doc.LinkEndChild( navg );
	...
	if(!nodes.empty()){
		...
		for(iter = nodes.begin(); iter != nodes.end(); iter++){
			...
			navg->LinkEndChild(node);

			node->SetAttribute("id", (*iter).getId());
			node->SetAttribute("x", (*iter).getPosition().getX());
			node->SetAttribute("y", (*iter).getPosition().getY());
			node->SetAttribute("enabled", (int) (*iter).isEnabled());

			TiXmlElement * ady = NULL;
			vector<int>::iterator iIter;
			for(iIter = (*iter).getAdyacence().begin() + 1; iIter < (*iter).getAdyacence().end(); iIter++){
				ady = new TiXmlElement("adyacent");
				node->LinkEndChild(ady);
				ady->SetAttribute("id", *iIter);
			}
		}
	}
...
void NavGraph::loadFile(string name){
	...
	if (loadOkay) {
		...
		TiXmlElement * nodo = doc.RootElement()->FirstChildElement();

		while(nodo != NULL){

			int x = 0, y = 0, id = 0;

			nodo->QueryIntAttribute("x", &x);
			nodo->QueryIntAttribute("y", &y);
			nodo->QueryIntAttribute("id", &id);

			ASNode node(Point(x,y));
			node.setId(id);

			TiXmlElement * ady = nodo->FirstChildElement()->FirstChildElement();
			while(ady != NULL){
				ady->QueryIntAttribute("id", &id);
				node.addEdge(id);
				ady = ady->NextSiblingElement();
			}
...
void NavGraph::changeNodeState(int id, bool enabled){
	vector<ASNode>::iterator asIter;
	for(asIter = nodes.begin(); asIter != nodes.end(); asIter++){
		if((*asIter).getId() == id){
			(*asIter).isEnabled(enabled);
			break;
		}
	}
}
