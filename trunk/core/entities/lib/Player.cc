...
Player::applyRecompensa() {
	ApplyObject(recompensa);
	if(recompensa >= (int)recompensas.size())
	{
		recompensa = recompensas.size()-1;
		if(recompensa < 0)
			recompensa = 0;
	}
}
void Player::addIdRecompensa()
{

	if(recompensas.size() > 0)
	{
		recompensa++;
		recompensa = recompensa%recompensas.size();
	}
}
