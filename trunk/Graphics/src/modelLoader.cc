...
const string PARAM1 = "-m";			//Paramétro para la opción "Carga desde modelos/"
const string DEFAULT = "c.3ds";		//Modelo por defecto.
...
const string TXT_TITLE = "Cargador de Modelos";
...
void makeTree() {
	...
	txt_puntos = new Text(Graphics::Font::Monospace, Point(-395,260,0), Color<float>(0.8,0.8,0.8,1), 14, ss.str());
	txt_modelo = new Text(Graphics::Font::Monospace, Point(-395,280,0), Color<float>(0.8,0.8,0.8,1), 14, TXT_MODELO + ruta);
...
void Init(int argc, char *argv[]) {
	...
	//cout << M_NAME << ruta << endl;

	color = Color<float>(0.1,0.1,0.1);
	inicializarOpenGL();
