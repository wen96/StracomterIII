...
SceneManager::SceneManager(GraphicTree* _scene) {
	...
	currentShader = Graphics::Shader::Pruebas;
	shadersActive = false;
	myClipping = true;
}
...
SceneManager::~SceneManager() {
	...
	currentShader = Graphics::Shader::None;
	shadersActive = false;
}
...
//					nodo->Render();
	...
	else if (tree == Graphics::StracomerTrees::gameMenu)
	{
		...
		glDisable(GL_BLEND);
	}else if (tree == Graphics::StracomerTrees::gameEditor){
		setZbuffer(true);
		setCulling(false);

		nodo = getNode(Graphics::DirectNodesEditor::c_escena);
		if(nodo != NULL)
		{
			if(nodo->getVisible())
			{
				nodo->Render();
				nodo = getNode(Graphics::DirectNodesEditor::t_escena);
				if(nodo != NULL)
					nodo->Render();
			}
		}
		...
		nodo = getNode(Graphics::DirectNodesEditor::c_menu);
		if(nodo != NULL)
		{
			if(nodo->getVisible())
			{
				nodo->Render();
				nodo = getNode(Graphics::DirectNodesEditor::t_menu);
				if(nodo != NULL)
					nodo->Render();
			}
		}
...
void SceneManager::addElement(Element *elem, GraphicNode* raiz)
{
	...
	if(elem != NULL)
	{
		...
		if(elem->getElementType() == Core::Element::e_entity)
		{
			...
			else if(((Entity*)elem)->getType() == Core::Entities::e_floor)
			{
				vector<Point> points = ((Floor*)elem)->getPerimetro();
				pol = new Shape(Polygon(points));
				scene->Insertar(pol, getNode(Graphics::DirectNodes::t_mapa));
			...
			/*if(((Entity*)elem)->getType() == Core::Entities::e_floor)
			{
				vector<Point> points = ((Obstacle*)elem)->getPuntos();
				pol = new Shape(Polygon(points));
				scene->Insertar(pol, getNode(Graphics::DirectNodes::t_mapa));
			}*/
		}
...
SceneManager::makeStracomterTree() {
	...
	//Cámara de la escena
	Camera *c_escena = new Camera(width, height, 1, 20000, Point());
	c_escena->setAngle(45);
	c_escena->setPerspective();

	Camera *c_mapa = new Camera(200, 150, 1, 2000, Point(width-200,height-150));
	c_mapa->setScale(1000);
	c_mapa->setOrtho();

	Camera *c_hud = new Camera(width, height, 0.0001, 20000, Point());
	c_hud->setScale(400);
	c_hud->setOrtho();

	Color<float> amb(0.1, 0.1, 0.1, 1.0);
	Color<float> dif(0.7, 0.7, 0.7, 1.0);
	Color<float> esp(0.3, 0.3, 0.3, 1.0);

	Point pos0(0, 0, 700); //Y ==> N/S; Z ==> A/B; X ==> I/D;
	Point pos1(0, 0, 700);
	Point pos2(0, 0, 100);
	...
	vector<Point> puntos;
	puntos.push_back(Point(height,0));
	puntos.push_back(Point(width,0));
	puntos.push_back(Point(width,150));
	puntos.push_back(Point(height,150));
	...
	puntosInfo.push_back(Point(width,height));
	puntosInfo.push_back(Point(width,height-150));
	puntosInfo.push_back(Point(width-150,height-150));
	puntosInfo.push_back(Point(width-150,height));
	...
	TForm *recompensa = new TForm(50,50, Point(774,524));
	recompensa->addWidget(new TPicture(Point(750,500), 48, 48, texturesroot + "null.png"));
	recompensa->setColor(Color<float>(0.2,0.2,0.2,0.7));


	t_hud->addScale(1, -1, 1);
	t_hud->addTranslation(-1.0 * width / 2, -1.0 * height / 2, -1);
	*t_menu = *t_hud;
	*t_console = *t_hud;
	...
	directNodes[Graphics::DirectNodes::t_recompensa] = recompensa->getNodo();
...
GraphicNode*
SceneManager::getNode(Graphics::DirectNodesEditor::Index index) {
...
SceneManager::makeMenuTree() {
	...
	Camera *c_menu = new Camera(width, height, 0.0001, 20000, Point());
	c_menu->setScale(400);
	c_menu->setOrtho();

	Transform *t_menu = new Transform();
	t_menu->addScale(1, -1, 1);
	t_menu->addTranslation(-1.0 * width / 2, -1.0 * height / 2, -1);

	Transform *t_efect = new Transform();
	t_efect->addScale(1, -1, 1);
	t_efect->addTranslation(-1.0 * width / 2, -1.0 * height / 2, -1);
...
void SceneManager::makeEditorTree() {
	if (scene != NULL) {
		delete scene;
		scene = NULL;
	}
	...
	directNodes = vector<GraphicNode*>(Graphics::DirectNodesEditor::size, NULL);
	...
	Transform *t_escena = new Transform();
//	t_escena->addScale(1, -1, 1);
//	t_escena->addTranslation(-1.0 * 800 / 2, -1.0 * 600 / 2, -1);
	...
	directNodes[Graphics::DirectNodesEditor::c_escena] = c_escena->getNodo();
	directNodes[Graphics::DirectNodesEditor::t_escena] = t_escena->getNodo();
	directNodes[Graphics::DirectNodesEditor::c_menu] = c_menu->getNodo();
	directNodes[Graphics::DirectNodesEditor::t_menu] = t_menu->getNodo();
	...
	tree = Graphics::StracomerTrees::gameEditor;
}

void SceneManager::removeEditorTree() {

}

Graphics::Shader::Name SceneManager::getCurrentShader() {
	return currentShader;
}

void SceneManager::setCurrentShader(Graphics::Shader::Name sh) {
	currentShader = sh;
}

bool SceneManager::isShaderActive() {
	return shadersActive;
}

void SceneManager::setShaderActive(bool shA) {
	shadersActive = shA;
}
