...
void Body::applyImpulse(Vector2D f, Point p){
	b2Vec2 auxF;
	auxF.x = cos(f.Angle() *M_PI / 180.0);
	auxF.y = sin(f.Angle() *M_PI / 180.0);

	b2Vec2 auxP;
	auxP.x = p.getX();
	auxP.y = p.getY();

	body->ApplyLinearImpulse(auxF, auxP);
}
...
void Body::setBodydata(BodyData *bodydata)
{
	...
	body = world->getWorld()->CreateBody(bodydata->getBodyDef());
	body->CreateFixture(&bodydata->getFixtureDef());
	body->SetMassData(bodydata->getMass());
	body->SetFixedRotation(false);
}
