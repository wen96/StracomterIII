...
Polygon::Polygon(Point center, std::vector<Point> vPuntos) {
	...
	for (int i = 0; i < n_vertices; i++) {
		puntos.push_back(Point(vPuntos[i].getX() + center.getX(), vPuntos[i].getY() + center.getY(), vPuntos[i].getZ() + center.getZ()));
	}
	radio = 0;
...
void Polygon::generatePoints() {
	...
	for (int i = 0; i < n_vertices; i++) {
		...
		puntos.push_back(Point(x, y, centro.getZ()));
	}
...
bool Polygon::isClockwise() const {
	...
	vector<Point> aux = puntos;
	aux.push_back(aux[0]);

	double suma = 0;

	vector<Point>::iterator pIter;
	for (pIter = aux.begin(); pIter != aux.end() - 1; pIter++) {
		suma += ((*(pIter + 1)).getX() - (*pIter).getX()) * ((*(pIter + 1)).getY() + (*pIter).getY());
	}
...
void Polygon::makeConvexHull() {
	...
	if (puntos.size() >= 4) {
		...
		for (iter = puntos.begin() + 1; iter != puntos.end(); iter++) {
			if ((*iter).getX() < leftMostPoint.getX() || ((*iter).getX() == leftMostPoint.getX() && (*iter).getY() < leftMostPoint.getY())) {
				leftMostPoint = *iter;
			}
		}
...
Point Polygon::getNewPoint(Point A, Point B, Point C, bool dextrogiro, double distance) {
...
void Polygon::toGPC(gpc_polygon *gpcPoly) {
	...
	gpcPoly->hole[0] = 0;
	gpcPoly->contour[0].vertex = (gpc_vertex*) malloc(this->n_vertices * sizeof(gpc_vertex));
...
void Polygon::fromGPC(gpc_polygon *gpcPoly) {
	...
	for (int i = 0; i < this->n_vertices; i++) {
		puntos.push_back(Point(gpcPoly->contour[0].vertex[i].x, gpcPoly->contour[0].vertex[i].y));
	}
}
...
void Polygon::Expand(double dist, bool dir){

	puntos.insert(puntos.begin(), puntos.end() - 1, puntos.end());
	puntos.insert(puntos.end(), puntos.begin()+1, puntos.begin() + 2);

//	puntos.insert(puntos.begin(), puntos.end()-1, puntos.end());
//	puntos.push_back(puntos[1]);

	vector<Point> vAux;
	vector<Point>::iterator piter;
	Point p;

	for(piter = puntos.begin() + 1; piter != puntos.end() - 1; piter++){

		p = Polygon::getNewPoint(*(piter - 1), *piter, *(piter + 1), dir, dist);
