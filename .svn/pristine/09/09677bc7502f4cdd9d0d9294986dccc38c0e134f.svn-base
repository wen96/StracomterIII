...
int
GraphicNode::getId() const{
	return id;
}

bool
GraphicNode::getVisible() const {
	return visible;
}

void
GraphicNode::setId(int id_) {
	id = id_;
}

void
GraphicNode::setVisible(bool visible_) {
	visible = visible_;
}
...
void
GraphicTree::PreordenAux(vector<Element*>& aux) const{
...
bool
GraphicTree::Existe(Element *element) const {
...
bool
GraphicTree::ExisteAux(Element *element, bool &existe) const {
...
void
GraphicTree::BorrarElemento(Element *elemento) const {
...
void
GraphicTree::BorrarRama(Element* elem) {
...
void
GraphicTree::BorrarRama(GraphicNode* nodo) {
...
GraphicNode::Render() {
	...
	if(item != NULL && visible)
	{
		...
		case Core::Element::e_entity:
			...
			Polygon *pol;
			t = ((Entity*)item)->getType();

			if(t == Core::Entities::e_wall){
				vector<Point> vaux = (((Wall*)item))->getPoints();
				for(int i = 0; i < 4; i++) vaux.pop_back();
				pol = new Polygon(vaux);
				DrawPolygon(*pol, Color<float>(1,0,0));
			}
			else if(t == Core::Entities::e_floor){
				pol = new Polygon(((Floor *)item)->getPoints());
				DrawPolygon(*pol, Color<float>(1,1,1));
			}
			else if(t == Core::Entities::e_pointer){


				pos = ((Pointer*)item)->getCenter();
				pos.setX(pos.getX() + 400);
				pos.setY(pos.getY() + 300);
				pos = ((Pointer*)item)->getAbsolutCenter(Point(), ((Pointer*)item)->getCenter(), 800,600);
				////cout << "POINTER: " << pos << endl;
				if(item->getPadre() != NULL)
				{
					((Transform*)item->getPadre()->getItem())->setIdentity();
					((Transform*)item->getPadre()->getItem())->addTranslation(pos);
				}
			//glLoadIdentity();
			...
			{
				pos = ((Entity*)item)->getCenter();
				angle = ((Entity*)item)->getAngle();
				////cout << pos << endl;
				if(item->getPadre() != NULL)
				{
					((Transform*)item->getPadre()->getItem())->setIdentity();
					((Transform*)item->getPadre()->getItem())->addTranslation(pos);
					((Transform*)item->getPadre()->getItem())->addRotationZ(angle);
				}
			//glLoadIdentity();
			//glTranslated(pos.getX(), pos.getY(), pos.getZ());
