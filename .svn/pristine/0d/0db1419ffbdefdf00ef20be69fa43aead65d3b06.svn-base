...
void TWindow::openSFML(bool full){
	if (screen==NULL){
		if (full){
			screen = new sf::RenderWindow(sf::VideoMode::GetDesktopMode(),nombreVentana,sf::Style::Fullscreen);
			fullscreen=full;
		}else{
			screen = new sf::RenderWindow(sf::VideoMode::GetDesktopMode(),nombreVentana,sf::Style::Titlebar | sf::Style::Close);
			fullscreen=full;
		}
		screen->Display();
	}
...
void TWindow::openSFML() {
	if (screen==NULL){
		screen = new sf::RenderWindow(sf::VideoMode::GetDesktopMode(),nombreVentana);
		screen->Display();
		fullscreen=false;
	}
}
...
void TWindow::openSFML(int width, int height) {
	if (screen==NULL){
		screen = new sf::RenderWindow(sf::VideoMode(width,height),nombreVentana);
		screen->Display();
		fullscreen=false;
	}
}
...
void TWindow::openSFML(int width, int height, bool full) {
	if (screen==NULL){
		if (full){
			screen = new sf::RenderWindow(sf::VideoMode(width,height),nombreVentana,sf::Style::Fullscreen);
			fullscreen=full;
		}else{
			screen = new sf::RenderWindow(sf::VideoMode(width,height),nombreVentana,
...
void TWindow::create(unsigned int width,unsigned int height, bool full){
	if (screen!= NULL) {
		fullscreen = full;
		if (full){
			screen->Create(sf::VideoMode(width,height),nombreVentana,sf::Style::Fullscreen);
		}else{
			screen->Create(sf::VideoMode(width,height),nombreVentana);
		}
