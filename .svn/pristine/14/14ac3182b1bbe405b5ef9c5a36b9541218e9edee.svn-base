...
void Enemy::Patrol(EntityManager* manager){
	...
	if(!meAtacan(manager)){
		if(enemySpotted(manager)){
			...
			if(patrolPosition.isInRadius(this->getCenter(), 10)){
				this->partialRotation();
			}else{
				if(path.empty()){
					this->calculatePath(patrolPosition);
				}
				if(this->Move())
					flagPatrullo = true;
			}
...
void Enemy::partialRotation(){
	...
	if(((int)ang) % 90 == 0){
		if(flagPatrullo){
			...
			if(timerPatrolRotation.GetElapsedTime() > 3000){
				flagPatrullo = true;
				this->turnAround(1);
...
Enemy::Enemy(const int idG, const Point pos, float angle,
