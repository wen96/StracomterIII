...
void Bot::Dispara(Entity *objetivo){
	...
	ResourceManager::getInstance()->getEvents()->doAttack(this);
}
...
int Bot::isInsideFOV(Point p) {
	...
	if(Point::distance(pos,p) < visionDistance){
		Vector2D v1(this->getAngle());
		Vector2D v2(p - pos);

		if(abs(v1.Angle(v2)) < visionAngle){
			result = 1;
		}
	}
...
Entity * Bot::selectObjetive(EntityManager *manager) {
	...
	while (liter != memory.end()) {
		...
		if (posibleObjetivo != NULL && !posibleObjetivo->isDead() && posibleObjetivo->isVisible()) { // Si el entity no esta muerto
