...
void Triangulation::createBorders(Point i, Point f) {
	...
	//cout << "Puntos iniciales: " << pxpy << pxny << nxpy << nxny << endl;
...
void Triangulation::addPoint(Point p) {
	...
	for (iter = puntos.begin(); iter != puntos.end() && !found; iter++) {
		if (*iter == p)
			found = true;
	}
...
void Triangulation::stepDelaunay() {
	...
	if (!puntos.empty()) {
		...
		for (triter = triangles.begin(); triter != triangles.end() && !encontrado; triter++) {
			int rel = (*triter)->isInside(puntos[0]);

			//if(rel == 0) cout << puntos[0] << " en " << **triter << " da " << rel << endl;
